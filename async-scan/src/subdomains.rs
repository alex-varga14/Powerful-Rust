...
pub async fn enumerate(http_client: &Client, target: &str) -> Result<Vec<Subdomain>, Error> {
    ...
    let mut subdomains: HashSet<String> = entries
        ...
        .collect();
    // Note flatten() is flatten() method of an Iterator, not a Future
    subdomains.insert(target.to_string());

    // Check if domains resolve by turning the subdaomins into a Stream.
    // By turning subdomains into a Stream, we can use the "map" combinator and "buffer_unordered" 
    // to scan the subdomains concurrently 
